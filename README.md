...
顶部项目下拉框对应 KeyHub 的项目名，工具仍使用素材文件夹名匹配该项目。新增的 3:1 分类中，前两个按产品标签 slug 定位，其余按页面 slug 定位；页面和产品标签会读取全部 REST 分页。`oem` 会先匹配同名页面，找不到时兼容 `oem-odm`；其他目标不存在时会记录并跳过，避免写错位置。
...
KeyHub 中多条项目记录使用同一个公网网址时，工具会使用 TXT 中唯一且同网址的项目名消歧；只有项目名仍不唯一或名称与网址互相冲突时才会阻止执行。
...
单张图片或单个项目发生错误时，批次不会自动停止，而是记录问题后继续处理后续分类和项目。批次自然结束后，如存在上传失败、写入失败、目标缺失或缺图项，界面会保留“重跑问题项”入口，只处理未成功的分类，已经成功的分类不会重复上传。

站点没有对应页面或产品标签时，该分类显示“目标不存在，已跳过”，不会作为红色待修复项反复重跑。通过兼容别名成功写入时，项目卡片显示实际目标 slug，例如 `oem-odm · 3:1`；素材分类本身仍保持 `oem-3-1-` 文件前缀。
